...
admin_externalpage_setup('tooluploadcourseURL');

$importid         = optional_param('importid', '', PARAM_INT);
$previewrows = optional_param('previewrows', 10, PARAM_INT);

$returnurl = new moodle_url('/admin/tool/uploadcourseURL/index.php');

if (empty($importid)) {
    $mform1 = new tool_uploadcourse_step1_form();
    if ($form1data = $mform1->get_data()) {
        $importid = csv_import_reader::get_new_iid('uploadcourseURL');
        $cir = new csv_import_reader($importid, 'uploadcourseURL');
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('uploadcourses', 'tool_uploadcourseURL'), 'uploadcourses', 'tool_uploadcourseURL');
        $mform1->display();
        echo $OUTPUT->footer();
        die();
    }
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    if (isset($form2data->showpreview)) {
        echo $OUTPUT->heading(get_string('uploadcoursespreview', 'tool_uploadcourseURL'));
        $processor->preview($previewrows, new tool_uploadcourse_tracker(tool_uploadcourse_tracker::OUTPUT_HTML));
        $mform2->display();
    } else {
        echo $OUTPUT->heading(get_string('uploadcoursesresult', 'tool_uploadcourseURL'));
